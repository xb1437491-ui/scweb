export const products = [
  {
    id: 1,
    name: '中式极简泡茶杯',
    category: 'cup',
    markets: ['UK', 'FR', 'DE', 'DK'],
    price: 29.9,
    tags: ['中式美学', '环保', '耐用'],
    description: '以中国茶文化为核心，融合欧洲极简审美的泡茶杯。',

    hero: {
      image: '/images/teacup.jpg',
      alt: '中式极简泡茶杯',
    },

    highlights: [
      {
        title: '极简器型',
        text: '线条克制、日常耐看，融入欧洲家居风格。',
      },
      {
        title: '环保耐用',
        text: '强调可持续与长生命周期使用体验。',
      },
      {
        title: '泡茶一体',
        text: '满足“热饮 + 慢生活”场景，适配居家与办公。',
      },
    ],

    marketFit: [
      {
        code: 'DE',
        title: '德国：理性务实 + 功能耐用',
        text: '突出耐用性与功能性；设计语言更偏“形式追随功能”的简洁理性。',
      },
      {
        code: 'DK',
        title: '丹麦：Hygge 松弛感 + 极简自然',
        text: '适配“热饮 + 慢节奏”的生活哲学；偏好天然、可循环与简约。',
      },
      {
        code: 'FR',
        title: '法国：理性决策 + 性价比 + 可持续',
        text: '强调环保、耐用、可持续，并在表达上突出“值不值、用多久”。',
      },
      {
        code: 'UK',
        title: '英国：礼品属性 + 文化氛围感',
        text: '用东方茶文化做差异化，强调生活方式与礼品场景。',
      },
    ],

    cultureStory: {
      title: '中国养生与茶：把“日常的平衡”带进生活',
      text:
        '茶不只是饮品，更是一种节奏。在忙碌里留一段安静的时间，我们用更克制的设计语言，把中式茶文化的“松弛与平衡”融入欧洲日常。',
    },
  },

  {
    id: 2,
    name: '可降解环保茶包',
    category: 'teabag',
    markets: ['FR', 'DE', 'DK'],
    price: 9.9,
    tags: ['环保', '可持续', '法规友好'],
    description: '采用可降解材料的一次性茶包，兼顾便利性与环保责任。',

    hero: {
      image: '/images/teabag.jpg',
      alt: '可降解环保茶包',
    },

    highlights: [
      {
        title: '可降解材料',
        text: '减少塑料使用，符合欧洲环保法规趋势。',
      },
      {
        title: '使用便捷',
        text: '一次性设计，适合快节奏生活场景。',
      },
      {
        title: '环境友好',
        text: '兼顾日常便利与生态责任。',
      },
    ],

    marketFit: [
      {
        code: 'DE',
        title: '德国：法规意识强 + 环保理性',
        text: '突出材料合规性与可持续生产，符合德国消费者的理性决策习惯。',
      },
      {
        code: 'DK',
        title: '丹麦：环保生活方式',
        text: '强调减少浪费、轻量化包装，与北欧可持续生活理念一致。',
      },
      {
        code: 'FR',
        title: '法国：性价比 + 绿色消费',
        text: '在环保基础上强调“好用且负担得起”，更易被日常消费者接受。',
      },
    ],

    cultureStory: {
      title: '从“随手一包茶”开始的环保选择',
      text:
        '在中国，饮茶既是习惯也是节制。我们希望通过可降解茶包，让便利不再以环境为代价，把东方的节制观念转化为现代环保实践。',
    },
  },

  {
    id: 3,
    name: '文创陶瓷杯',
    category: 'cup',
    markets: ['UK', 'FR', 'DE', 'DK'],
    price: 79.9,
    tags: ['文创', '陶瓷', '手工'],
    description: '一款融合传统文化与现代设计的手工陶瓷杯，适合家庭、办公室与收藏。',

    hero: {
      image: '/images/ceramic-cup.jpg',
      alt: '文创陶瓷杯',
    },

    highlights: [
      {
        title: '手工制作',
        text: '每只陶瓷杯由匠人手工打造，细节精致，体现工艺美学。',
      },
      {
        title: '文化传承',
        text: '每款陶瓷杯都融合传统文化元素，寓意深远，作为礼品非常合适。',
      },
      {
        title: '现代设计',
        text: '简洁的外形设计，符合现代审美，并适合多场景使用。',
      },
    ],

    marketFit: [
      {
        code: 'UK',
        title: '英国：礼品属性 + 文化表达',
        text: '英国市场对文化和艺术类产品有较强需求，陶瓷杯作为中式文化的象征，能吸引对“东方文化”有兴趣的消费者。',
      },
      {
        code: 'FR',
        title: '法国：艺术氛围 + 细节体验',
        text: '法国消费者重视艺术气息，陶瓷杯的设计和手工工艺，符合他们对艺术的追求。',
      },
      {
        code: 'DE',
        title: '德国：功能性 + 精致工艺',
        text: '德国市场偏好有功能性和实用性的文化产品，陶瓷杯的质感与手工制作正契合其审美需求。',
      },
      {
        code: 'DK',
        title: '丹麦：极简设计 + 传统文化',
        text: '丹麦市场对极简设计有高度偏好，陶瓷杯的传统文化元素能与当地的极简生活理念相匹配。',
      },
    ],

    cultureStory: {
      title: '融合传统文化的现代陶瓷艺术',
      text:
        '陶瓷杯不仅仅是一件饮品容器，更是文化的载体。从东方茶道到当代艺术，每一只杯子背后都有一个故事。我们通过结合手工制作与现代设计，将传统文化与当代生活方式完美融合。',
    },
  },


  {
    id: 4,
    name: '天然木质养生梳',
    category: 'accessory',
    markets: ['DE', 'DK'],
    price: 15.9,
    tags: ['天然材质', '养生', '极简设计'],
    description: '采用天然木材制作的养生梳，适合日常护理与头部放松，强调自然与长期使用价值。',

    hero: {
      image: '/images/wooden-comb.jpg',
      alt: '天然木质养生梳',
    },

    highlights: [
      {
        title: '天然木材',
        text: '选用天然木材制作，触感温润，减少静电，适合长期使用。',
      },
      {
        title: '日常养生',
        text: '通过梳头这一日常动作，帮助放松头部与颈部，融入轻养生理念。',
      },
      {
        title: '极简外观',
        text: '造型克制、无多余装饰，适合北欧与现代家居环境。',
      },
    ],

    marketFit: [
      {
        code: 'DE',
        title: '德国：理性消费 + 功能耐用',
        text: '强调产品耐用性、天然材料与实用价值，符合德国消费者对“用得久”的偏好。',
      },
      {
        code: 'DK',
        title: '丹麦：极简生活方式 + 自然材料',
        text: '北欧市场偏好自然、低干预的生活用品，木质养生梳与当地极简生活理念高度契合。',
      },
    ],

    cultureStory: {
      title: '从日常梳头开始的身体关照',
      text:
        '在东方生活里，梳头不仅是整理仪容，更是一种照顾身体的日常仪式。我们用天然木材与克制设计，让这一简单动作重新回到现代生活中。',
    },
  },

]
